...
namespace {
		...
		switch (format) {
			...
			default:
				throw std::invalid_argument(
				    "Vulkan padded buffer copy does not support this format"
				);
		}
	}
	...
	vk::AccessFlags LegacyAccess(vk::AccessFlags2 value) noexcept {
		return vk::AccessFlags(
		    static_cast<vk::AccessFlags::MaskType>(static_cast<vk::AccessFlags2::MaskType>(value))
		);
	}
	...
	NativeAccess MapAccess(ResourceUsage usage, AccessMode mode) noexcept {
		if (HasUsage(usage, ResourceUsage::ColorAttachment)) {
			return {
			    ...
			    mode == AccessMode::Read ? vk::AccessFlagBits2::eColorAttachmentRead :
		...
		if (HasUsage(usage, ResourceUsage::DepthStencilAttachment)) {
			return {
			    ...
			    mode == AccessMode::Read ? vk::AccessFlagBits2::eDepthStencilAttachmentRead :
			                               vk::AccessFlagBits2::eDepthStencilAttachmentRead |
			            vk::AccessFlagBits2::eDepthStencilAttachmentWrite,
			    mode == AccessMode::Read ? vk::ImageLayout::eDepthStencilReadOnlyOptimal :
		...
		if (HasUsage(usage, ResourceUsage::Storage)) {
			return {
			    ...
			    mode == AccessMode::Read      ? vk::AccessFlagBits2::eShaderStorageRead :
			        mode == AccessMode::Write ? vk::AccessFlagBits2::eShaderStorageWrite :
	...
	vk::ImageSubresourceRange NativeRange(
		...
		return vk::ImageSubresourceRange(
		    ...
		    0u,
		    texture.image_type == vk::ImageType::e3D ? 1u :
	...
	vk::SharedSurfaceKHR CreateSurface(
		...
		vk::AndroidSurfaceCreateInfoKHR info(vk::AndroidSurfaceCreateFlagsKHR{}, target);
...
#endif
		return vk::SharedSurfaceKHR(native, instance, {nullptr, dispatcher});
	}
	...
	std::pair<vk::PresentModeKHR, bool> SelectPresentMode(
		...
		auto modes =
		    scheduler->physical_device->getSurfacePresentModesKHR(*surface, *scheduler->dispatcher);
		...
		scheduler->physical_device->getFeatures2(&device_features, *scheduler->dispatcher);
		auto fifo_latest_ready_supported = fifo_features.presentModeFifoLatestReady &&
		...
		if (vertical_sync) {
			...
			return {
			    fifo_latest_ready_supported ? vk::PresentModeKHR::eFifoLatestReady :
	...
	PresentationContext::SwapChain CreateSwapChain(
		...
		auto capabilities =
		    scheduler->physical_device->getSurfaceCapabilitiesKHR(*surface, *scheduler->dispatcher);
		...
		auto formats =
		    scheduler->physical_device->getSurfaceFormatsKHR(*surface, *scheduler->dispatcher);
		auto source_format = static_cast<vk::Format>(source.format);
		auto format =
		    std::ranges::find_if(formats, [source_format](vk::SurfaceFormatKHR const& candidate) {
		...
		auto native = scheduler->device->createSwapchainKHR(info, nullptr, *scheduler->dispatcher);
		...
		result.back_buffers =
		    scheduler->device->getSwapchainImagesKHR(native, *scheduler->dispatcher);
		...
		for (std::size_t index = 0u; index < result.back_buffers.size(); ++index) {
			...
			result.synchronization.emplace_back(
			    ...
			    vk::SharedFence(fence, scheduler->device, {nullptr, *scheduler->dispatcher}),
			    0u
			);
		}
	...
	struct Recorder {
		...
		void operator()(BeginRendering const& value) {
			...
			if (!dynamic_rendering_supported) {
				...
				attachments.reserve(value.colors.size() * 2u + (value.depth_stencil ? 1u : 0u));
				...
				for (auto const& color : value.colors) {
					...
					attachments.emplace_back(
					    ...
					    color.load == LoadOperation::Load      ? vk::AttachmentLoadOp::eLoad :
					        color.load == LoadOperation::Clear ? vk::AttachmentLoadOp::eClear :
					                                             vk::AttachmentLoadOp::eDontCare,
					    color.store == StoreOperation::Store ? vk::AttachmentStoreOp::eStore :
					...
					if (color.resolve_view) {
						...
						auto resolve_attachment = static_cast<std::uint32_t>(attachments.size());
						attachments.emplace_back(
						    vk::AttachmentDescriptionFlags{},
						    resolve_view.format,
				...
				if (value.depth_stencil) {
					...
					depth_reference.emplace(attachment, depth_layout);
					framebuffer_views.emplace_back(*NativeImageView(view));
					clear_values.emplace_back(
				...
				auto native_render_pass =
				    device->createRenderPass(render_pass_info, nullptr, dispatcher);
				vk::SharedRenderPass render_pass(native_render_pass, device, {nullptr, dispatcher});
				if (!framebuffer_extent) {
					throw std::invalid_argument(
					    "Vulkan rendering requires at least one attachment"
					);
				}
				...
				auto native_framebuffer =
				    device->createFramebuffer(framebuffer_info, nullptr, dispatcher);
		...
		void operator()(BindResourceGroup const& value) {
			...
			std::ranges::transform(
			    ...
			    [&](std::size_t index) {
				    ...
				    if (offset > group.dynamic_buffers[index].maximum_offset ||
		...
		void operator()(BindVertexBuffer const& value) {
			...
			commands.bindVertexBuffers(value.slot, 1u, &native_buffer, &offset, dispatcher);
		}
		...
		void operator()(BindIndexBuffer const& value) {
			...
			commands.bindIndexBuffer(
			    ...
			    value.offset,
			    value.type == IndexType::Uint16 ? vk::IndexType::eUint16 : vk::IndexType::eUint32,
			    dispatcher
			);
		}
		...
		void operator()(Viewport const& value) {
			...
			vk::Viewport
			    viewport(value.x, y, value.width, height, value.minimum_depth, value.maximum_depth);
			commands.setViewport(0u, viewport, dispatcher);
		}
		...
		void operator()(CopyBuffer const& value) {
			...
			vk::BufferCopy region(value.source_offset, value.destination_offset, value.size);
			commands.copyBuffer(
		...
		void operator()(WriteBuffer const& value) {
			auto& buffer = NativeBuffer(resources[value.resource].get());
			auto buffer_size =
			    std::get<std::size_t>(resource_metadata[value.resource].size_or_extent);
			...
			buffer.allocation.Write(value.offset, value.data);
		}
		...
		void operator()(Present const& value) {
			...
			auto& work = (*presentation_works)[(*presentation_indices)[presentation_cursor++]];
...
namespace fyuu_rhi::vulkan {
	CompletionToken::ManagedCommandBuffer::~ManagedCommandBuffer() noexcept {
		...
		try {
			...
			if (auto timeline = std::get_if<TimelineCompletion>(&completion)) {
				...
				vk::SemaphoreWaitInfo wait_info(vk::SemaphoreWaitFlags{}, 1u, &semaphore, &value);
				(void)context->device->waitSemaphores(
				    wait_info,
				    (std::numeric_limits<std::uint64_t>::max)(),
				    *context->dispatcher
				);
			} else if (auto binary = std::get_if<vk::SharedFence>(&completion)) {
			...
			(void)impl.reset(vk::CommandBufferResetFlags{}, *context->dispatcher);
			// Give the buffer back to its own pool, then return that pool to the family's idle
			// queue, so the next batch needing one can take it. The map is never erased from, so a
			// plain lookup by family is enough and no address is held anywhere.
			std::unique_lock<std::mutex> pools_lock(context->command_pools_mutex);
			pool.command_buffers.emplace_back(impl);
			context->command_pools[family].emplace_back(std::move(pool));
		} catch (...) {
	...
	BinaryCompletionPointAllocator::ManagedBinaryCompletionPoint::
		...
		try {
			...
			owner->device->resetFences(*fence, *owner->dispatcher);
	...
	BinaryCompletionPointAllocator::ManagedBinaryCompletionPoint BinaryCompletionPointAllocator::
	    AllocateCompletionPoint(std::size_t consumed_semaphore_count) {
		...
		if (!fence) {
			auto native_fence = device->createFence(vk::FenceCreateInfo{}, nullptr, *dispatcher);
			fence = vk::SharedFence(native_fence, device, {nullptr, *dispatcher});
		}
		while (consumed_semaphores.size() < consumed_semaphore_count) {
			auto native_semaphore =
			    device->createSemaphore(vk::SemaphoreCreateInfo{}, nullptr, *dispatcher);
			vk::SharedSemaphore semaphore(native_semaphore, device, {nullptr, *dispatcher});
...
namespace fyuu_rhi {
	template <> struct CreateScheduler<vulkan::LogicalDevice> {
		...
		execution::CommandScheduler operator()() const {
			...
			if (context.timeline_semaphore_supported) {
				context.completion_points.emplace<vulkan::CommandSchedulerContext::TimelineMap>();
			} else {
			...
			return execution::MakeCommandScheduler(std::move(context));
		}
	};
} // namespace fyuu_rhi

namespace fyuu_rhi::execution {
	template <> struct ExecuteCommands<vulkan::CommandSchedulerContext> {
		vulkan::CommandSchedulerContext* context;
		...
		CompletionToken operator()(
			...
			auto FindPresentation = [&](std::uint32_t family) -> PresentationContext* {
				std::unique_lock<std::mutex> presentations_lock(scheduler.presentations_mutex);
			...
			std::ranges::transform(
			    ...
			    [](fyuu_rhi::PipelineResourceGroup const& group) -> vulkan::PipelineResourceGroup& {
				    ...
				    auto native = std::get_if<vulkan::PipelineResourceGroup>(&group.m_impl->native);
				    if (!native) {
					    throw std::invalid_argument(
			...
			for (std::size_t index = 0u; index < plan.batches.size(); ++index) {
				...
				if (batch.id != index) {
					throw std::invalid_argument(
					    "Vulkan execution batch IDs must match storage indices"
					);
				}
				for (auto dependency : batch.dependencies) {
					if (dependency >= index) {
						throw std::invalid_argument(
						    "Vulkan execution batches are not topologically ordered"
						);
					}
				}
				for (auto const& node : batch.nodes) {
					if (node.queue != batch.queue) {
						throw std::invalid_argument(
						    "Vulkan execution node and batch queue types differ"
						);
					}
					for (auto const& command : node.commands) {
						if (std::holds_alternative<BeginRendering>(command) &&
							...
							throw std::invalid_argument(
							    "Vulkan BeginRendering requires a graphics queue"
							);
						}
						...
						if (supported.empty()) {
							throw std::runtime_error(
							    "No Vulkan queue family supports the presentation surface"
							);
						}
						if (pinned_family) {
							if (std::ranges::find(supported, *pinned_family) == supported.end()) {
								throw std::runtime_error(
								    "Cached Vulkan presentation family no longer supports its "
									"surface"
								);
							}
						...
						if (allowed_families[index].empty()) {
							...
							std::erase_if(allowed_families[index], [&](std::uint32_t family) {
								return std::ranges::find(supported, family) == supported.end();
							});
							if (allowed_families[index].empty()) {
								throw std::runtime_error(
								    "No Vulkan queue family supports every target in one "
									"presentation batch"
								);
							}
						}
			...
			for (auto& resource : resources) {
				if (auto binary = std::get_if<vk::SharedFence>(&resource.get().completion)) {
			...
			auto reservation =
			    scheduler.queue_allocator.Reserve(scheduler.device, scheduler.dispatcher, requests);
			...
			for (std::size_t resource = 0u; resource < resources.size(); ++resource) {
				...
				if (native.owner_family != (std::numeric_limits<std::uint32_t>::max)()) {
				    ...
				    resource < plan.first_accesses.size() && !plan.first_accesses[resource].empty()
				) {
					resource_anchors[resource] =
					    queue_identifiers[plan.first_accesses[resource].front().batch].family;
				}
			}
			...
			for (std::size_t index = 0u; index < plan.batches.size(); ++index) {
				...
				CommandPoolContext pool;
				{
					...
					auto& idle_pools = scheduler.command_pools[identifier.family];
					if (!idle_pools.empty()) {
						// Take an idle pool out of the family's queue. A separate pool per batch is
						// what lets the recording pass below run on several threads at once, since
						// Vulkan only forbids two buffers from one pool being recorded together.
						pool = std::move(idle_pools.front());
						idle_pools.pop_front();
					} else {
					...
					if (!pool.command_buffers.empty()) {
						...
						commands =
						    scheduler.device
						        ->allocateCommandBuffers(allocate_info, *scheduler.dispatcher)
						        .front();
					}
				}
				...
				prepared.emplace_back(&plan.batches[index], command_index);
			}
			std::vector<PresentationWork> presentation_works;
			...
			for (auto& preparation : presentation_preparations) {
				...
				if (std::ranges::find(preparation.supported_families, family) ==
					...
					throw std::logic_error(
					    "QueueAllocator violated Vulkan present-family restrictions"
					);
				}
				// The presentations map is keyed by queue family, so this lock only guards
				// the insertion of one PresentationContext; it is never held while a
				// swapchain operation or a blocking WSI wait runs.
				...
				// Waiting for a free frame slot and acquiring its image is the blocking part
				// of this preparation. It is scoped to this one PresentationContext, so a
				// window presenting another target never queues behind it.
				vk::SharedSwapchainKHR acquired_swapchain;
				vk::Image acquired_image;
				vk::Extent2D acquired_extent{0u, 0u};
				vk::SharedSemaphore image_available;
				vk::SharedFence retirement_fence;
				std::uint32_t acquired_image_index = 0u;
				bool acquired_maintenance1_supported = false;
				{
					std::unique_lock<std::mutex> presentation_lock(presentation->mutex);
					if (presentation->cache.Contains(preparation.target)) {
						...
						preparation.surface = presentation->cache.Get(preparation.target).surface;
					}
					...
					if (!recreate) {
						...
						recreate = current.out_of_date || current.present_family != family ||
					...
					if (recreate) {
						...
						presentation->cache.Put(preparation.target, std::move(replacement));
					}
					...
					while (!stop_token.stop_requested()) {
						...
						synchronization_index = (swapchain->last_back_buffer_index + 1u) %
						...
						acquired = scheduler.device->acquireNextImageKHR(
						    ...
						    *synchronization->acquire_semaphore,
						    swapchain->swapchain_maintenance1_supported ? nullptr :
						                                                  *synchronization->fence,
						    *scheduler.dispatcher
						);
					...
					// The frame slot state is committed while its context is still locked.
					acquired_swapchain = swapchain->impl;
					acquired_image = swapchain->back_buffers[acquired->value];
					acquired_extent = swapchain->extent;
					image_available = synchronization->acquire_semaphore;
					retirement_fence = synchronization->fence;
					acquired_image_index = acquired->value;
					acquired_maintenance1_supported = swapchain->swapchain_maintenance1_supported;
				...
				// Publication only needs the values captured above, so it runs unlocked.
				...
				presentation_works.emplace_back(
				    ...
				    acquired_maintenance1_supported
				);
			...
			for (std::size_t resource = 0u; resource < plan.first_accesses.size(); ++resource) {
				if (auto timeline =
				        std::get_if<TimelineCompletion>(&resources[resource].get().completion)) {
					for (auto const& access : plan.first_accesses[resource]) {
			...
			if (scheduler.timeline_semaphore_supported) {
				...
				for (std::size_t index = 0u; index < prepared.size(); ++index) {
					...
					if (inserted) {
						...
						auto native =
						    scheduler.device->createSemaphore(info, nullptr, *scheduler.dispatcher);
				...
				token.binary_completions.reserve(prepared.size() + plan.last_accesses.size());
				for (std::size_t index = 0u; index < prepared.size(); ++index) {
				...
				for (std::size_t index = 0u; index < prepared.size(); ++index) {
					auto destination = reservation.GetQueue(index).GetIdentifier();
					auto& completion = token.binary_completions[*prepared[index].binary_completion];
			...
			for (std::size_t resource = 0u; resource < plan.last_accesses.size(); ++resource) {
				...
				bool multiple_queues = std::ranges::any_of(terminals, [&](std::size_t batch) {
				...
				if (scheduler.timeline_semaphore_supported) {
					...
					auto incoming =
					    std::ranges::count_if(join.terminal_batches, [&](std::size_t batch) {
						    return queue_identifiers[batch] != target_queue;
					    });
			...
			ParallelFor(std::size_t{0u}, prepared.size(), [&](std::size_t index) {
				if (cancelled.load(std::memory_order_acquire) || stop_token.stop_requested()) {
					cancelled.store(true, std::memory_order_release);
					return;
				}
				try {
					// No lock here: every batch was leased its own command pool during the serial
					// preparation pass, so batches recording concurrently never share a pool.
					// Vulkan only forbids recording two buffers from the *same* pool at once.
					...
					for (auto const& node : prepared[index].plan->nodes) {
						recorder.node = &node;
						for (
						    std::size_t resource = 0u; resource < plan.first_accesses.size();
						    ++resource
						) {
							for (auto const& access : plan.first_accesses[resource]) {
						...
						for (auto const& barrier : prepared[index].plan->barriers) {
							...
							auto destination_family =
							    queue_identifiers[barrier.destination_batch].family;
							EmitBarrier(
							    commands,
						...
						for (
						    std::size_t resource = 0u; resource < plan.last_accesses.size();
						    ++resource
						) {
							for (auto const& access : plan.last_accesses[resource]) {
								if (access.batch != index || access.node != node.id) {
			...
			for (std::size_t index = 0u; index < prepared.size(); ++index) {
				auto identifier = reservation.GetQueue(index).GetIdentifier();
				auto [queue, inserted] =
				    queue_indices.try_emplace(identifier, queue_batches.size());
				if (inserted) {
			...
			ParallelFor(std::size_t{0u}, queue_batches.size(), [&](std::size_t queue_index) {
				...
				// One description per batch. Every batch keeps its own wait and signal
				// semaphores, its own timeline values, and its own binary fence; only the
				// vkQueueSubmit call is shared. std::deque, not std::vector: a SubmitInfo
				// points at the member containers of its unit, so those addresses must
				...
				std::deque<BatchSubmit> submits;
				// Units carried by the next vkQueueSubmit. A batch owning a binary fence is
				// never merged: vkQueueSubmit signals exactly one fence for all of its
				// submit infos, so a shared fence would retire those batches too early.
				std::vector<std::size_t> merged;

				auto Publish = [&](BatchSubmit const& unit) {
					auto index = unit.batch;
					if (unit.timeline) {
						token.command_buffers[prepared[index].command_buffer].completion =
						    *prepared[index].timeline;
					} else {
						auto& completion =
						    token.binary_completions[*prepared[index].binary_completion];
				...
				auto Fail = [&](std::size_t index, std::exception_ptr error) {
					prepared[index].submission_error = std::move(error);
					submission_states[index].store(2u, std::memory_order_release);
					submission_states[index].notify_all();
				};
				// Issues every fence-less unit handed to merged as one vkQueueSubmit and
				// publishes them individually once the call returned.
				auto Flush = [&]() {
					if (merged.empty()) {
						return;
					}
					std::vector<vk::SubmitInfo> infos;
					infos.reserve(merged.size());
					for (auto position : merged) {
						infos.emplace_back(submits[position].submit_info);
					}
					std::size_t published = 0u;
					try {
						queue.GetNativeQueue()->submit(infos, vk::Fence{}, *scheduler.dispatcher);
						for (; published < merged.size(); ++published) {
							Publish(submits[merged[published]]);
						}
					} catch (...) {
						auto error = std::current_exception();
						for (auto position = published; position < merged.size(); ++position) {
							Fail(submits[merged[position]].batch, error);
						}
					}
					merged.clear();
				};
				...
				for (auto index : batches) {
					...
					for (auto dependency : prepared[index].plan->dependencies) {
						...
						if (state == 0u &&
						    reservation.GetQueue(dependency).GetIdentifier() !=
						        queue.GetIdentifier()) {
							// This queue's already prepared batches must reach the queue
							// before this thread blocks: the producer may itself be waiting
							// for one of them. Batches sharing this queue are covered by
							// submission order inside the merged call.
							Flush();
							while (state == 0u) {
								...
								state =
								    submission_states[dependency].load(std::memory_order_acquire);
							}
						}
						if (state == 2u) {
					...
					try {
						auto& unit = submits.emplace_back();
						unit.batch = index;
						unit.timeline = prepared[index].timeline.has_value();
						unit.waits = prepared[index].waits;
						unit.signals = prepared[index].signals;
						if (unit.timeline) {
							// Vulkan requires pWaitSemaphores, pWaitDstStageMask and
							// pWaitTimelineSemaphoreValues to be index-parallel. The waits already
							// collected above are binary semaphores and ignore their value, so seed
							// one placeholder per binary wait *before* appending the timeline waits.
							// Padding the tail instead would shift every timeline value onto a binary
							// semaphore and leave the timeline semaphores waiting on value 0, which
							// waits for nothing.
							unit.wait_values.assign(unit.waits.size(), 0u);
							for (auto const& external : prepared[index].external_waits) {
								unit.waits.emplace_back(*external.semaphore);
								unit.wait_values.emplace_back(external.value);
							}
							for (auto dependency : prepared[index].plan->dependencies) {
								if (reservation.GetQueue(dependency).GetIdentifier() ==
								    queue.GetIdentifier()) {
									continue;
								}
								unit.waits.emplace_back(*prepared[dependency].timeline->semaphore);
								unit.wait_values.emplace_back(prepared[dependency].timeline->value);
							}
							unit.signals.emplace_back(*prepared[index].timeline->semaphore);
							unit.signal_values.resize(unit.signals.size(), 0u);
							unit.signal_values.back() = prepared[index].timeline->value;
							unit.timeline_info = vk::TimelineSemaphoreSubmitInfo(
							    unit.wait_values,
							    unit.signal_values
							);
						}
						unit.wait_stages.assign(
						    unit.waits.size(),
						    vk::PipelineStageFlagBits::eAllCommands
						);
						unit.command_buffers.emplace_back(
						    token.command_buffers[prepared[index].command_buffer].impl
						);
						unit.submit_info = vk::SubmitInfo(
						    unit.waits,
						    unit.wait_stages,
						    unit.command_buffers,
						    unit.signals
						);
						unit.submit_info.pNext = unit.timeline ? &unit.timeline_info : nullptr;
						if (prepared[index].binary_completion) {
							unit.fence =
							    *token.binary_completions[*prepared[index].binary_completion].fence;
						}
						if (unit.fence) {
							// A binary fence belongs to one submission unit only.
							Flush();
							queue.GetNativeQueue()->submit(
							    unit.submit_info,
							    unit.fence,
							...
							merged.emplace_back(submits.size() - 1u);
						}
					} catch (...) {
						Fail(index, std::current_exception());
					}
				}
				Flush();
			});
			...
			if (!scheduler.timeline_semaphore_supported) {
				...
				for (std::size_t index = 0u; index < prepared.size(); ++index) {
					...
					try {
						...
						vk::SubmitInfo cleanup(waits, stages, {}, {});
						queue.GetNativeQueue()->submit(
						    cleanup,
						    *completion.fence,
						    *scheduler.dispatcher
			...
			for (std::size_t index = 0u; index < prepared.size(); ++index) {
				if (prepared[index].timeline &&
				    submission_states[index].load(std::memory_order_acquire) == 1u) {
			...
			if (!presentation_works.empty()) {
				...
				auto has_abandoned_acquire =
				    std::ranges::any_of(presentation_works, [&](PresentationWork const& work) {
				...
				for (auto const& [identifier, work_indices] : present_groups) {
					try {
						auto& queue =
						    reservation.GetQueue(presentation_works[work_indices.front()].batch);
						std::unique_lock<std::mutex> queue_lock(queue.GetSubmissionMutex());
						std::vector<vk::Semaphore> waits;
						...
						vk::PresentInfoKHR present_info(waits, swapchains, image_indices, results);
						...
						if (use_present_fences) {
							...
							present_fence_info.swapchainCount =
							    static_cast<std::uint32_t>(present_fences.size());
							present_fence_info.pFences = present_fences.data();
							present_info.pNext = &present_fence_info;
						}
						auto aggregate =
						    queue.GetNativeQueue()->presentKHR(present_info, *scheduler.dispatcher);
						for (
						    std::size_t position = 0u; position < work_indices.size(); ++position
						) {
			...
			if (!token.exception) {
				...
				for (auto& join : resource_joins) {
					try {
						...
						vk::TimelineSemaphoreSubmitInfo timeline_info;
						if (join.timeline) {
							// Index-parallel with waits, exactly as above: the pre-existing waits
							// are binary semaphores and get value placeholders first.
							wait_values.assign(waits.size(), 0u);
							for (auto batch : join.terminal_batches) {
						...
						vk::SubmitInfo submit_info(waits, stages, {}, signals);
						...
						queue.GetNativeQueue()->submit(submit_info, fence, *scheduler.dispatcher);
						if (join.timeline) {
			...
			for (std::size_t resource = 0u; resource < plan.last_accesses.size(); ++resource) {
				...
				auto terminals_submitted = std::ranges::all_of(
				    ...
				    [&](ExecutionBoundaryAccess const& access) {
					    return submission_states[access.batch].load(std::memory_order_acquire) ==
					        1u;
				    }
				);
				...
				auto joined =
				    std::ranges::find_if(resource_joins, [resource](ResourceJoin const& join) {
					    return join.resource == resource;
				    });
				auto completion_batch =
				    joined == resource_joins.end() ? last.batch : joined->target_batch;
				...
				if (device_drained_after_error) {
					...
					native.completion = joined->timeline ?
					    ...
					    CompletionPoint(token.binary_completions[*joined->binary_completion].fence);
				} else if (prepared[completion_batch].timeline) {
					native.completion = *prepared[completion_batch].timeline;
				} else {
					native.completion =
					    token.binary_completions[*prepared[completion_batch].binary_completion]
					        .fence;
				}
			}
			return MakeCompletionToken(std::move(token));
