...
namespace {
	...
	VKAPI_ATTR vk::Bool32 VKAPI_CALL DebugMessenger(
		...
		try {
			auto message = std::format(
			    "Vulkan [{}{}{}] {} ({}): {}",
			    types & vk::DebugUtilsMessageTypeFlagBitsEXT::eGeneral ? "General " : "",
			    types & vk::DebugUtilsMessageTypeFlagBitsEXT::eValidation ? "Validation " : "",
			    types & vk::DebugUtilsMessageTypeFlagBitsEXT::ePerformance ? "Performance" : "",
	...
	bool AddLayer(
		...
		if (!available_layers.contains(layer_name)) {
			fyuu_rhi::log::Warning(
			    std::format("Vulkan instance layer '{}' is unavailable", layer_name)
			);
			return false;
		}
	...
	void AddOptionalExtension(
		...
		if (!available_extensions.contains(ext_name)) {
			fyuu_rhi::log::Warning(
			    std::format("Optional Vulkan instance extension '{}' is unavailable", ext_name)
			);
			return;
		}
	...
	void AddMandatoryExtension(
		...
		static constexpr std::array common_extensions{vk::KHRSurfaceExtensionName};
#if defined(_WIN32)
		static constexpr std::array platform_extensions{vk::KHRWin32SurfaceExtensionName};
#elif defined(__ANDROID__)
		static constexpr std::array platform_extensions{vk::KHRAndroidSurfaceExtensionName};
...
namespace fyuu_rhi {
	template <> struct CreateInstance<vulkan::Instance> {
		vulkan::Instance operator()() const {
			...
			auto supported_layers = layer_properties |
			    std::views::transform([](vk::LayerProperties const& property) -> std::string_view {
				                        return property.layerName;
			                        }) |
			    std::ranges::to<std::unordered_set>();
			validation_layer_enabled =
			    AddLayer(supported_layers, enabled_layers, "VK_LAYER_KHRONOS_validation");
			...
			if (supported_extensions.contains(vk::KHRGetSurfaceCapabilities2ExtensionName)) {
				AddOptionalExtension(
				    ...
				    supported_extensions,
				    enabled_extensions,
				    supported_extensions.contains(vk::KHRSurfaceMaintenance1ExtensionName) ?
			...
			auto native_instance = vk::createInstance(
				create_info,
				nullptr,
				vk::detail::defaultDispatchLoaderDynamic
			);
			...
			auto stored_extensions = enabled_extensions |
			    std::views::transform([](char const* extension) {
				                         return std::string_view(extension);
			                         }) |
			    std::ranges::to<std::unordered_set>();
			auto stored_layers = enabled_layers | std::views::transform([](char const* layer) {
	...
	template <> struct EnumeratePhysicalDevices<vulkan::Instance> {
		...
		std::vector<PhysicalDevice> operator()() const {
			...
			return native_devices | std::views::transform([this](vk::PhysicalDevice native) {
