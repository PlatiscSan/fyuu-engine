...
namespace {
	...
	D3D12_RESOURCE_FLAGS NativeResourceFlags(ResourceFlags const& flags) noexcept {
		...
		bool is_unordered_access = flags.Test(Bits::StorageTexelBuffer) ||
		    flags.Test(Bits::StorageBuffer) || flags.Test(Bits::StorageBinding) ||
		    flags.Test(Bits::StorageAttachment);
		if (is_unordered_access) {
			result |= D3D12_RESOURCE_FLAG_ALLOW_UNORDERED_ACCESS;
		}
		if (flags.Test(Bits::RenderAttachment)) {
			bool is_depth_stencil = flags.Test(Bits::D16Unorm) ||
			    flags.Test(Bits::D24UnormS8Uint) || flags.Test(Bits::D32Float) ||
			    flags.Test(Bits::D32FloatS8X24Uint);
			result |= is_depth_stencil ? D3D12_RESOURCE_FLAG_ALLOW_DEPTH_STENCIL :
	...
	D3D12_CLEAR_VALUE ClearValue(DXGI_FORMAT format) noexcept {
		...
		if (fyuu_rhi::d3d12::IsDepthStencil(format)) {
			result.DepthStencil = {.Depth = 1.0f, .Stencil = 0u};
		} else {
	...
	Microsoft::WRL::ComPtr<ID3D12RootSignature> CreateRootSignature(
		...
		if (serialized.empty()) {
				...
				auto AddRangeAndParameter = [&ranges, &parameters, &entry](
					...
					parameters.push_back(
					    {.ParameterType = D3D12_ROOT_PARAMETER_TYPE_DESCRIPTOR_TABLE,
					     .DescriptorTable =
					         {.NumDescriptorRanges = 1, .pDescriptorRanges = &ranges.back()},
			...
			for (auto const& range : pipeline_interface.push_constants) {
				if (range.offset != 0 || range.size == 0 ||
					...
					throw std::invalid_argument(
					    "D3D12 push constants must be a non-empty, four-byte-aligned range "
			...
			if (FAILED(result)) {
				auto message = errors ? std::string_view(
				                            static_cast<char const*>(errors->GetBufferPointer()),
				                            errors->GetBufferSize()
				                        ) :
				                        std::string_view{};
				throw std::runtime_error(
				    std::format("D3D12 root signature serialization failed: {}", message)
				);
			}
		...
		d3d12::ThrowIfFailed(logical_device->CreateRootSignature(
		    0,
		    serialized.data(),
		    serialized.size(),
		    IID_PPV_ARGS(&result)
	...
	slang::TargetDesc MakeDXILTarget(Microsoft::WRL::ComPtr<ID3D12Device> const& logical_device) {
		...
		HRESULT hr = logical_device->CheckFeatureSupport(
		...
		struct LUIDHasher {
			std::size_t operator()(LUID const& key) const noexcept {
				return static_cast<std::size_t>(key.LowPart) ^
				    (static_cast<std::size_t>(static_cast<std::uint32_t>(key.HighPart))
				     << (sizeof(std::size_t) * 4));
			}
		};
		struct LUIDEqual {
...
namespace fyuu_rhi {
	...
	template <> struct CreateBuffer<d3d12::LogicalDevice> {
		...
		Resource operator()(std::size_t size_in_bytes, ResourceFlags const& flags) const {
			...
			auto resource_descriptor =
			    CD3DX12_RESOURCE_DESC::Buffer(size_in_bytes, NativeResourceFlags(flags));
			Microsoft::WRL::ComPtr<D3D12MA::Allocation> allocation;
			d3d12::ThrowIfFailed(logical_device->memory_allocator->CreateResource(
	...
	template <> struct CreateTexture<d3d12::LogicalDevice> {
		d3d12::LogicalDevice* logical_device;
		...
		Resource operator()(
			...
			if (flags.Test(Bits::Texture1D)) {
				...
				resource_descriptor = CD3DX12_RESOURCE_DESC::Tex2D(
				    ...
				    static_cast<UINT16>(mip_levels),
				    sample_count,
				    SampleQuality(logical_device->rm_tracker.GetDevice(), format, sample_count),
				    resource_flags,
				    texture_layout
				);
			}
			...
			auto optimized_clear_value =
			    flags.Test(Bits::RenderAttachment) ? &clear_value : nullptr;
			Microsoft::WRL::ComPtr<D3D12MA::Allocation> allocation;
			d3d12::ThrowIfFailed(logical_device->memory_allocator->CreateResource(
			    &allocation_descriptor,
			    &resource_descriptor,
	...
	template <> struct CreateSampler<d3d12::LogicalDevice> {
		d3d12::LogicalDevice* logical_device;
		...
		Sampler operator()(SamplerDescriptor const& descriptor) const {
			...
			logical_device->rm_tracker.GetDevice()->CreateSampler(&sampler_desc, handle.CPU());
			return MakeSampler(d3d12::Sampler{std::move(handle)});
		}
	};

	template <> struct CreateGraphicsPipeline<d3d12::LogicalDevice> {
		d3d12::LogicalDevice* logical_device;
		...
		Pipeline operator()(pipeline::GraphicsPipelineDescriptor const& descriptor) const {
			...
			auto target = MakeDXILTarget(device);
			auto adapter = d3d12::GetPhysicalDevice(device);
			...
			for (auto const& entry : program.GetEntryPoints()) {
				...
				switch (entry.stage) {
					...
					case pipeline::Stage::Fragment:
						native.PS = bytecode;
						break;
					case pipeline::Stage::TessellationControl:
						native.HS = bytecode;
						break;
					case pipeline::Stage::TessellationEvaluation:
						native.DS = bytecode;
						break;
			...
			std::vector<D3D12_INPUT_ELEMENT_DESC> input_elements =
			    descriptor.vertex.attributes | std::views::transform([&](auto const& attribute) {
				    ...
				    if (reflected == reflected_inputs.end()) {
					    throw std::invalid_argument(
					        std::format(
					            "No reflected vertex input at location {}",
					            attribute.location
					        )
					    );
				    }
				    ...
				    return D3D12_INPUT_ELEMENT_DESC{
				        ...
				        .InstanceDataStepRate =
				            buffer->input_rate == pipeline::VertexInputRate::Vertex ? 0u : 1u
				    };
			    }) |
			...
			if (descriptor.primitive.strip_index_format) {
				if (descriptor.primitive.topology != pipeline::PrimitiveTopology::LineStrip &&
				...
				native.IBStripCutValue =
				    *descriptor.primitive.strip_index_format == pipeline::IndexFormat::Uint16 ?
			...
			native.RasterizerState.FrontCounterClockwise =
			    descriptor.rasterization.front_face == pipeline::FrontFace::CounterClockwise;
			...
			for (std::size_t index = 0; index < descriptor.color_targets.size(); ++index) {
				...
				if (target_state.blend) {
					...
					target_blend.DestBlend =
					    MapBlendFactor(target_state.blend->color.destination_factor);
					target_blend.BlendOp = MapBlendOperation(target_state.blend->color.operation);
					target_blend.SrcBlendAlpha =
					    MapBlendFactor(target_state.blend->alpha.source_factor);
					target_blend.DestBlendAlpha =
					    MapBlendFactor(target_state.blend->alpha.destination_factor);
					target_blend.BlendOpAlpha =
					    MapBlendOperation(target_state.blend->alpha.operation);
				}
			}
			...
			if (descriptor.depth_stencil) {
				...
				native.DepthStencilState.DepthWriteMask =
				    descriptor.depth_stencil->depth_write_enabled ? D3D12_DEPTH_WRITE_MASK_ALL :
				...
				native.DepthStencilState.StencilReadMask =
				    static_cast<UINT8>(descriptor.depth_stencil->stencil_read_mask);
				native.DepthStencilState.StencilWriteMask =
				    static_cast<UINT8>(descriptor.depth_stencil->stencil_write_mask);
				native.DepthStencilState.FrontFace =
				    MapStencilFace(descriptor.depth_stencil->stencil_front);
				native.DepthStencilState.BackFace =
				    MapStencilFace(descriptor.depth_stencil->stencil_back);
			}
			...
			auto creation_result = device->CreateGraphicsPipelineState(&native, IID_PPV_ARGS(&pso));
			if (FAILED(creation_result) && !cached_pso.empty()) {
				native.CachedPSO = {};
				cached_pso.clear();
				creation_result = device->CreateGraphicsPipelineState(&native, IID_PPV_ARGS(&pso));
			}
	...
	template <> struct CreateComputePipeline<d3d12::LogicalDevice> {
		d3d12::LogicalDevice* logical_device;
		...
		Pipeline operator()(pipeline::ComputePipelineDescriptor const& descriptor) const {
			...
			for (auto const& entry : program.GetEntryPoints()) {
				if (entry.stage != pipeline::Stage::Compute) {
					throw std::invalid_argument(
					    "D3D12 compute pipelines accept only a compute entry point"
					);
				}
				if (native.CS.pShaderBytecode) {
					throw std::invalid_argument(
					    "D3D12 compute pipelines require exactly one compute entry point"
					);
				}
			...
			if (!native.CS.pShaderBytecode) {
				throw std::invalid_argument(
				    "D3D12 compute pipelines require one compute entry point"
				);
			}
			...
			Microsoft::WRL::ComPtr<ID3D12PipelineState> pso;
			auto creation_result = device->CreateComputePipelineState(&native, IID_PPV_ARGS(&pso));
			if (FAILED(creation_result) && !cached_pso.empty()) {
				native.CachedPSO = {};
				cached_pso.clear();
				creation_result = device->CreateComputePipelineState(&native, IID_PPV_ARGS(&pso));
			}
			d3d12::ThrowIfFailed(creation_result);
			if (cached_pso.empty()) {
				Microsoft::WRL::ComPtr<ID3DBlob> blob;
	...
	template <> struct CreateScheduler<d3d12::LogicalDevice> {
		d3d12::LogicalDevice* logical_device;
		...
		execution::CommandScheduler operator()() const {
			...
			auto CreateQueue = [&](D3D12_COMMAND_LIST_TYPE type) {
				...
				d3d12::ThrowIfFailed(
				    device->CreateCommandQueue(&descriptor, IID_PPV_ARGS(&context->impl))
				);
				d3d12::ThrowIfFailed(
				    device->CreateFence(0u, D3D12_FENCE_FLAG_NONE, IID_PPV_ARGS(&context->fence))
				);
				if (type == D3D12_COMMAND_LIST_TYPE_DIRECT) {
					context->presentation_context =
					    std::make_unique<d3d12::QueueContext::PresentationContext>();
				}
				return context;
			};
			...
			return execution::MakeCommandScheduler(
			    d3d12::CommandSchedulerContext{std::move(queues)}
			);
		}
	};
