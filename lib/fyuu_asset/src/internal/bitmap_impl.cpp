...
namespace fyuu_asset {
	...
	std::size_t Bitmap::BytesPerPixel() const noexcept {
		switch (format) {
	...
	void Bitmap::Serialize(std::filesystem::path const& path) const {
		if (!Valid()) {
	...
	Bitmap Bitmap::Deserialize(std::filesystem::path const& path) {
