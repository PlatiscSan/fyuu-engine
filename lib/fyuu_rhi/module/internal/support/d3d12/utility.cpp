...
namespace fyuu_rhi::d3d12 {
	...
	D3D12_SRV_DIMENSION SRVDimension(fyuu_rhi::ResourceFlags const& flags) {
		...
		for (auto const& [flag, dimension] : dimensions) {
			...
			if (found) {
				throw std::invalid_argument("A D3D12 SRV requires exactly one texture view type");
			}
			found = true;
			result = dimension;
		}
	...
	D3D12_UAV_DIMENSION UAVDimension(fyuu_rhi::ResourceFlags const& flags) {
		...
		for (auto const& [flag, dimension] : dimensions) {
			...
			if (found) {
				throw std::invalid_argument("A D3D12 UAV requires exactly one texture view type");
			}
			found = true;
			result = dimension;
		}
	...
	D3D12_RTV_DIMENSION RTVDimension(fyuu_rhi::ResourceFlags const& flags) {
		...
		for (auto const& [flag, dimension] : dimensions) {
			...
			if (found) {
				throw std::invalid_argument("A D3D12 RTV requires exactly one texture view type");
			}
			found = true;
			result = dimension;
		}
	...
	D3D12_DSV_DIMENSION DSVDimension(fyuu_rhi::ResourceFlags const& flags) {
		...
		for (auto const& [flag, dimension] : dimensions) {
			...
			if (found) {
				throw std::invalid_argument("A D3D12 DSV requires exactly one texture view type");
			}
			found = true;
			result = dimension;
		}
	...
	D3D12_FILTER_TYPE MipmapFilterType(MipmapFilterMode mode) noexcept {
		return mode == MipmapFilterMode::Nearest ? D3D12_FILTER_TYPE_POINT :
	...
	D3D12_FILTER BuildFilter(
		...
		auto reduction = comparison ? D3D12_FILTER_REDUCTION_TYPE_COMPARISON :
	...
	struct ManagedEvent {
		...
		explicit ManagedEvent(wil::unique_event&& event) noexcept : impl(std::move(event)) {
		}
	...
	/// Reports a call failure that means the device is gone rather than the call itself
	/// is wrong.
	///
	/// A removed device announces itself through the HRESULT of the call that noticed
	/// it — Present is the documented example — which on its own does not say why the
	/// device went away. The device's own reason code does, so it becomes the error
	/// code and is folded into the message, which lets a caller tell "rebuild the
	/// device" apart from an ordinary presentation failure. Every other HRESULT keeps
	/// the behaviour of ThrowIfFailed.
	void ThrowIfDeviceLost(HRESULT result, Microsoft::WRL::ComPtr<ID3D12Device> const& device) {
		if (!FAILED(result)) {
			return;
		}
		...
		// The reason code is only authoritative once the device is gone: a failed
		// result carries the reason, S_OK means the removal has not been recorded yet,
		// and the failing call's own HRESULT is then the best answer available.
		HRESULT reason = device ? device->GetDeviceRemovedReason() : result;
		if (!FAILED(reason)) {
			reason = result;
		}
		_com_error error(reason);
		std::string description = boost::locale::conv::utf_to_utf<char>(error.ErrorMessage());
		throw std::system_error(
		    std::error_code(reason, std::system_category()),
		    std::format(
		        "D3D12 device lost: the call returned 0x{:08X}, GetDeviceRemovedReason returned "
				"0x{:08X} ({})",
	...
	Microsoft::WRL::ComPtr<ID3D12Device> GetLogicalDevice(
	...
	Microsoft::WRL::ComPtr<IDXGIAdapter1> GetPhysicalDevice(
	...
	Microsoft::WRL::ComPtr<IDXGIFactory2> GetInstance(
