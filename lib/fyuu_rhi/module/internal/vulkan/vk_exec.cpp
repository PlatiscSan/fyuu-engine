...
module fyuu_rhi:vulkan_execution;
#if !defined(__APPLE__)
#if defined(__cpp_lib_modules)
import std;
#endif // defined(__cpp_lib_modules)
...
namespace {
	...
	using fyuu_rhi::ClipSpace;
	using fyuu_rhi::vulkan::Backend;
	...
	Backend::Resource::Buffer& NativeBuffer(Backend::Resource& resource) {
		auto result = std::get_if<Backend::Resource::Buffer>(&resource.impl);
		if (!result) {
	...
	Backend::Resource::Texture& NativeTexture(Backend::Resource& resource) {
		auto result = std::get_if<Backend::Resource::Texture>(&resource.impl);
		if (!result) {
	...
	vk::ImageSubresourceRange NativeRange(
		...
		auto format = static_cast<vk::Format>(texture.tex_info.format);
		...
		return vk::ImageSubresourceRange(
			NativeAspect(format),
			0u,
			texture.tex_info.mipLevels,
			0u,
			texture.tex_info.arrayLayers
		);
	}
	...
	void EmitBarrier(
		...
		if (auto texture = std::get_if<Backend::Resource::Texture>(&resource.impl)) {
			...
			barrier.subresourceRange = NativeRange(*texture, range);
			if (synchronization2) {
				...
				legacy.image = texture->vk_handle;
		...
		if (synchronization2) {
			...
			legacy.srcQueueFamilyIndex = source_family;
			legacy.dstQueueFamilyIndex = destination_family;
			legacy.buffer = buffer.vk_handle;
	...
	vk::SharedSurfaceKHR CreateSurface(
		...
		vk::SurfaceKHR native = std::visit(
			[&](auto const& handle) -> vk::SurfaceKHR {
				...
				if constexpr (std::same_as<Handle, Backend::X11PlatformHandle>) {
	...
	std::pair<vk::PresentModeKHR, bool> SelectPresentMode(
		std::shared_ptr<Backend::SchedulerContext::Implementation> const& scheduler,
	...
	vk::Extent2D ResolveSwapchainExtent(
		...
		return vk::Extent2D(
			std::clamp(
				source.tex_info.extent.width,
				capabilities.minImageExtent.width,
				capabilities.maxImageExtent.width
			),
			std::clamp(
				source.tex_info.extent.height,
	...
	Backend::PresentationContext::SwapChain CreateSwapChain(
		std::shared_ptr<Backend::SchedulerContext::Implementation> const& scheduler,
		vk::SharedSurfaceKHR const& surface,
		Backend::Resource::Texture const& source,
		std::uint32_t family,
		...
		Backend::PresentationContext::SwapChain const* previous
	) {
		if (source.tex_info.imageType != VK_IMAGE_TYPE_2D ||
			source.tex_info.samples != VK_SAMPLE_COUNT_1_BIT ||
			source.tex_info.mipLevels != 1u ||
			source.tex_info.arrayLayers != 1u) {
		...
		auto source_format = static_cast<vk::Format>(source.tex_info.format);
		...
		Backend::PresentationContext::SwapChain result;
		...
		result.sync_objs.reserve(result.back_buffers.size());
	...
	struct Recorder {
		...
		void operator()(BeginRendering const& value) {
			...
			if (!dynamic_rendering_supported) {
				...
				auto include_attachment_extent = [&](Backend::Resource::Texture const& texture,
					Backend::View::Texture const& view) {
					auto mip = view.info.subresourceRange.baseMipLevel;
					vk::Extent2D extent(
						(std::max)(1u, texture.tex_info.extent.width >> mip),
						(std::max)(1u, texture.tex_info.extent.height >> mip)
					);
				...
				for (auto const& color : value.colors) {
					auto const& view = std::get<Backend::View::Texture>(views[color.view].get().impl);
					auto& texture = NativeTexture(resources[color.resource].get());
					include_attachment_extent(texture, view);
					rendering_color_formats.emplace_back(view.info.format);
					auto samples = static_cast<vk::SampleCountFlagBits>(texture.tex_info.samples);
					...
					attachments.emplace_back(
						...
						view.info.format,
						static_cast<vk::SampleCountFlagBits>(texture.tex_info.samples),
					...
					framebuffer_views.emplace_back(*view.impl);
					...
					if (color.resolve_view) {
						auto const& resolve_view = std::get<Backend::View::Texture>(
							views[*color.resolve_view].get().impl
						);
						auto& resolve_texture = NativeTexture(
							resources[*color.resolve_resource].get()
						);
						...
						framebuffer_views.emplace_back(*resolve_view.impl);
				...
				if (value.depth_stencil) {
					...
					auto const& view = std::get<Backend::View::Texture>(
						views[value.depth_stencil->view].get().impl
					);
					auto& texture = NativeTexture(
						resources[value.depth_stencil->resource].get()
					);
					include_attachment_extent(texture, view);
					rendering_depth_stencil_format = view.info.format;
					auto samples = static_cast<vk::SampleCountFlagBits>(texture.tex_info.samples);
					if (rendering_samples_set && rendering_samples != samples) {
						throw std::invalid_argument(
							"Vulkan rendering attachments have different sample counts"
			...
			for (auto const& color : value.colors) {
				...
				if (color.resolve_view) {
					auto const& resolve = std::get<Backend::View::Texture>(
						views[*color.resolve_view].get().impl
					);
			...
			if (value.depth_stencil) {
				...
				depth = attachment;
				if ((view.info.subresourceRange.aspectMask & vk::ImageAspectFlagBits::eStencil) !=
					vk::ImageAspectFlags{}) {
					stencil = attachment;
		...
		void operator()(BindVertexBuffer const& value) {
			...
			vk::Buffer native_buffer(buffer.vk_handle);
		...
		void operator()(WriteBuffer const& value) {
			auto& buffer = NativeBuffer(resources[value.resource].get());
			if (value.offset > buffer.buf_info.size ||
				value.data.size() > buffer.buf_info.size - value.offset) {
				throw std::out_of_range("Vulkan write exceeds the buffer size");
			}
			void* pointer = nullptr;
			auto result = vmaMapMemory(buffer.mem_alloc->impl, buffer.alloc, &pointer);
			if (result != VK_SUCCESS) {
				throw std::runtime_error(
					std::format(
						"vmaMapMemory failed: {}", static_cast<int>(result)
					)
				);
			}
			std::memcpy(
				static_cast<std::byte*>(pointer) + value.offset,
				value.data.data(),
				value.data.size()
			);
			vmaFlushAllocation(
		...
		void operator()(CopyTexture const& value) {
			...
			auto& destination = NativeTexture(resources[value.destination].get());
			auto source_format = static_cast<vk::Format>(source.tex_info.format);
			auto destination_format = static_cast<vk::Format>(destination.tex_info.format);
			vk::ImageCopy region(
				vk::ImageSubresourceLayers(
		...
		void operator()(Present const& value) {
			...
			if (work.extent.width == source.tex_info.extent.width &&
				work.extent.height == source.tex_info.extent.height) {
				vk::ImageCopy region(
					layers,
					vk::Offset3D{},
					layers,
				...
				vk::ImageBlit region(
					...
					{ vk::Offset3D{}, vk::Offset3D(
						static_cast<std::int32_t>(source.tex_info.extent.width),
						static_cast<std::int32_t>(source.tex_info.extent.height),
						1
					) },
					layers,
					{ vk::Offset3D{}, vk::Offset3D(
...
namespace fyuu_rhi::vulkan {
	...
	struct Backend::CompletionToken::Implementation {
		struct ManagedCommandBuffer {
			/// Keeps the scheduler-owned command pool alive until GPU retirement.
			std::shared_ptr<SchedulerContext::Implementation> owner;
			/// Selects the pool to which impl is returned. Command pools are bound to
			/// a queue family, not to an individual queue index.
			std::uint32_t family;
			vk::CommandBuffer impl;
			CompletionPoint completion;

			ManagedCommandBuffer(
				std::shared_ptr<SchedulerContext::Implementation> const& owner_,
				std::uint32_t family_,
				vk::CommandBuffer impl_
			) noexcept : owner(owner_), family(family_), impl(impl_) {}
		...
		std::shared_ptr<SchedulerContext::Implementation> owner;
		/// Legacy framebuffer/render-pass objects must outlive pending command buffers.
		/// Their declaration precedes command_buffers so reverse destruction waits
		/// for command-buffer retirement before destroying these referenced objects.
	...
	Backend::CompletionToken::Implementation::ManagedCommandBuffer::~ManagedCommandBuffer() noexcept {
		if (!owner || !impl) {
			return;
		}
		try {
			...
			if (auto timeline = std::get_if<TimelineCompletion>(&completion)) {
				...
				(void)owner->device->waitSemaphores(
					wait_info,
			...
			auto pool = owner->command_pools.find(family);
			if (pool == owner->command_pools.end()) {
				return;
			}
			std::unique_lock<std::mutex> lock(pool->second.mutex);
			pool->second.command_buffers.emplace_back(impl);
		}
		catch (...) {
	...
	void Backend::CompletionToken::Deleter::operator()(Implementation* impl) const noexcept {
		// Defined here so deletion only requires Implementation completeness in this
		// translation unit; the token can otherwise be destroyed through an
		// incomplete type.
		delete impl;
	}

	Backend::CompletionToken::~CompletionToken() noexcept {
		if (!impl || !impl->owner) {
			return;
		}
		try {
			for (auto const& fence : impl->presentation_fences) {
				(void)impl->owner->device->waitForFences(
					*fence,
					true,
					(std::numeric_limits<std::uint64_t>::max)(),
					*impl->owner->dispatcher
				);
			}
		}
		catch (...) {
	...
	bool Backend::CompletionToken::Poll() noexcept {
		...
		try {
			// Poll is deliberately non-blocking. Native device-loss errors are moved
			// into the token error channel so the scheduler completion operation can
			// finish instead of repeatedly polling an unrecoverable device.
			for (auto const& completion : impl->timeline_completions) {
				auto value = impl->owner->device->getSemaphoreCounterValue(
			...
			for (auto const& completion : impl->binary_completions) {
			...
			for (auto const& fence : impl->presentation_fences) {
				auto result = impl->owner->device->getFenceStatus(
					*fence,
					*impl->owner->dispatcher
				);
				if (result == vk::Result::eNotReady) {
	...
	std::exception_ptr Backend::CompletionToken::Error() const noexcept {
		return impl ? impl->exception : nullptr;
	}

	bool Backend::CompletionToken::IsStopped() const noexcept {
		return impl && impl->is_cancelled;
	}

	Backend::BinaryCompletionPointAllocator::ManagedBinaryCompletionPoint::~ManagedBinaryCompletionPoint() noexcept {
	...
	Backend::BinaryCompletionPointAllocator::ManagedBinaryCompletionPoint
		Backend::BinaryCompletionPointAllocator::AllocateCompletionPoint(std::size_t consumed_semaphore_count) {
	...
	Backend::SchedulerContext Backend::CreateScheduler(LogicalDevice const& ld) {
		auto implementation = std::make_shared<SchedulerContext::Implementation>(
			ld.phys_dev.impl,
			ld.impl,
			ld.dispatcher,
			ld.queue_alloc
		);
		implementation->timeline_semaphore_supported = ld.enabled_features.contains(
			vk::StructureType::ePhysicalDeviceTimelineSemaphoreFeatures
		);
		implementation->synchronization2_supported = ld.enabled_features.contains(
			vk::StructureType::ePhysicalDeviceSynchronization2Features
		);
		implementation->dynamic_rendering_supported = ld.enabled_features.contains(
			vk::StructureType::ePhysicalDeviceDynamicRenderingFeatures
		);
		if (implementation->timeline_semaphore_supported) {
			implementation->completion_points.emplace<SchedulerContext::Implementation::TimelineMap>();
		}
		else {
			auto allocator = std::make_shared<BinaryCompletionPointAllocator>();
			allocator->device = ld.impl;
			allocator->dispatcher = ld.dispatcher;
			implementation->completion_points = std::move(allocator);
		}
	...
	Backend::CompletionToken Backend::ExecuteCommands(
		...
		// Presentation-family affinity is scheduler state and must be observed before
		// QueueAllocator reserves the graph. Holding the graph transaction lock here
		// closes the gap in which another execution could create the same target on a
		// different family between cache inspection and reservation.
		...
		for (std::size_t index = 0u; index < plan.batches.size(); ++index) {
			...
			for (auto const& node : batch.nodes) {
				...
				for (auto const& command : node.commands) {
					...
					for (auto& [family, context] : scheduler.impl->presentations) {
						std::unique_lock<std::mutex> presentation_lock(context.mutex);
						if (context.cache.Contains(target)) {
							pinned_family = family;
							surface = context.cache.Get(target).surface;
							break;
						}
					}
					if (!surface) {
						surface = CreateSurface(
							scheduler.impl->physical_device,
							target,
							*scheduler.impl->dispatcher
						);
					}
					auto queue_families = scheduler.impl->physical_device->getQueueFamilyProperties(
						*scheduler.impl->dispatcher
					);
					std::vector<std::uint32_t> supported;
					for (std::uint32_t family = 0u; family < queue_families.size(); ++family) {
						if (scheduler.impl->physical_device->getSurfaceSupportKHR(
							family,
							*surface,
							*scheduler.impl->dispatcher
						)) {
		...
		// One token allocation owns every native retirement object produced below.
		// No batch-specific heap ownership is introduced.
		// make_unique cannot express the custom deleter, so allocate explicitly.
		// The stateless Deleter is default-constructed; destruction happens through
		// Deleter::operator(), which keeps Implementation incomplete at other sites.
		auto token_state = std::unique_ptr<CompletionToken::Implementation, CompletionToken::Deleter>(
			new CompletionToken::Implementation()
		);
		token_state->owner = scheduler.impl;
		CompletionToken token(std::move(token_state));
		if (stop_token.stop_requested()) {
			token.impl->is_cancelled = true;
			return token;
		}
		// Resource states are shared across schedulers. Address ordering gives every
		// execution the same lock order even when two plans bind resources at
		// different indices, preventing an AB/BA deadlock.
		std::vector<ResourceState*> resource_states;
		resource_states.reserve(resources.size());
		for (auto& resource : resources) {
			std::visit(
				[&](auto& native) {
					using Native = std::remove_cvref_t<decltype(native)>;
					if constexpr (!std::same_as<Native, std::monostate>) {
						resource_states.emplace_back(native.state.get());
					}
				},
				resource.get().impl
			);
		}
		std::ranges::sort(resource_states);
		resource_states.erase(
			std::ranges::unique(resource_states).begin(),
			resource_states.end()
		);
		std::vector<std::unique_lock<std::mutex>> resource_locks;
		resource_locks.reserve(resource_states.size());
		for (auto state : resource_states) {
			resource_locks.emplace_back(state->mutex);
			if (auto binary = std::get_if<BinaryCompletion>(&state->completion)) {
				...
				(void)scheduler.impl->device->waitForFences(
					*binary->fence,
					true,
					(std::numeric_limits<std::uint64_t>::max)(),
					*scheduler.impl->dispatcher
		...
		for (std::size_t resource = 0u; resource < plan.first_accesses.size(); ++resource) {
			...
			std::visit(
				[&](auto& native) {
					using Native = std::remove_cvref_t<decltype(native)>;
					if constexpr (!std::same_as<Native, std::monostate>) {
						state = native.state.get();
					}
				},
				resources[resource].get().impl
			);
			if (!state || state->owner_family == (std::numeric_limits<std::uint32_t>::max)()) {
				continue;
			}
			for (auto const& access : plan.first_accesses[resource]) {
				if (!requests[access.batch].preferred_family) {
					requests[access.batch].preferred_family = state->owner_family;
				}
			}
		}
		...
		auto reservation = scheduler.impl->queue_allocator.Reserve(
			scheduler.impl->device,
			scheduler.impl->dispatcher,
			requests
		);
		...
		for (std::size_t resource = 0u; resource < resources.size(); ++resource) {
			...
			if (state && state->owner_family !=
				(std::numeric_limits<std::uint32_t>::max)()) {
				resource_anchors[resource] = state->owner_family;
		...
		token.impl->command_buffers.reserve(plan.batches.size());
		for (std::size_t index = 0u; index < plan.batches.size(); ++index) {
			if (stop_token.stop_requested()) {
				token.impl->is_cancelled = true;
				return token;
			...
			auto [pool, inserted] = scheduler.impl->command_pools.try_emplace(identifier.family);
			if (inserted) {
				vk::CommandPoolCreateInfo info(
					vk::CommandPoolCreateFlagBits::eResetCommandBuffer,
					identifier.family
				);
				auto native_pool = scheduler.impl->device->createCommandPool(
					info,
					nullptr,
					*scheduler.impl->dispatcher
				);
				pool->second.impl = vk::SharedCommandPool(
					native_pool,
					scheduler.impl->device,
					{
						nullptr,
			...
			vk::CommandBuffer commands;
			{
				std::unique_lock<std::mutex> pool_lock(pool->second.mutex);
				if (!pool->second.command_buffers.empty()) {
					commands = pool->second.command_buffers.front();
					pool->second.command_buffers.pop_front();
				}
				else {
					...
					commands = scheduler.impl->device->allocateCommandBuffers(
						allocate_info,
						*scheduler.impl->dispatcher
					).front();
				}
			}
			auto command_index = token.impl->command_buffers.size();
			token.impl->command_buffers.emplace_back(
				scheduler.impl,
				identifier.family,
				commands
			);
		...
		for (auto& preparation : presentation_preparations) {
			...
			auto [presentation, inserted] = scheduler.impl->presentations.try_emplace(family);
			(void)inserted;
			std::unique_lock<std::mutex> presentation_lock(presentation->second.mutex);
			auto& source = NativeTexture(resources[preparation.command.source].get());
			if (presentation->second.cache.Contains(preparation.target)) {
				...
				preparation.surface = presentation->second.cache.Get(
					preparation.target
				).surface;
			}
			...
			bool recreate = !presentation->second.cache.Contains(preparation.target);
			Backend::PresentationContext::SwapChain const* previous = nullptr;
			if (!recreate) {
				auto& current = presentation->second.cache.Get(preparation.target);
				previous = &current;
				auto capabilities = scheduler.impl->physical_device->getSurfaceCapabilitiesKHR(
					*preparation.surface,
					*scheduler.impl->dispatcher
				);
				...
				recreate = current.out_of_date ||
					...
					current.format != static_cast<vk::Format>(source.tex_info.format) ||
			...
			if (recreate) {
				...
				if (previous) {
					if (previous->swapchain_maintenance1_supported) {
						...
						for (auto const& synchronization : previous->sync_objs) {
							(void)scheduler.impl->device->waitForFences(
						...
						scheduler.impl->device->waitIdle(
							*scheduler.impl->dispatcher
						);
					}
				}
				presentation->second.cache.Put(
					preparation.target,
					std::move(replacement)
				);
			}
			auto& swapchain = presentation->second.cache.Get(preparation.target);
			auto synchronization_index = (
				swapchain.last_back_buffer_index + 1u
			) % swapchain.sync_objs.size();
			auto& synchronization = swapchain.sync_objs[synchronization_index];
			(void)scheduler.impl->device->waitForFences(
				*synchronization.fence,
				true,
				(std::numeric_limits<std::uint64_t>::max)(),
			...
			scheduler.impl->device->resetFences(
				*synchronization.fence,
				*scheduler.impl->dispatcher
			);
			// With swapchain maintenance1 the frame-slot fence belongs to Present:
			// acquire signals only image_available, while queuePresentKHR signals the
			// fence after the presentation engine has finished using this image. The
			// fallback path has no present fence extension, so acquire temporarily uses
			// the fence to make image_available's reuse observable on the host.
			auto acquired = scheduler.impl->device->acquireNextImageKHR(
				*swapchain.impl,
				(std::numeric_limits<std::uint64_t>::max)(),
				*synchronization.acquire_semaphore,
				swapchain.swapchain_maintenance1_supported ? *synchronization.fence : nullptr,
				*scheduler.impl->dispatcher
			);
			if (acquired.result == vk::Result::eErrorOutOfDateKHR) {
				swapchain.out_of_date = true;
				throw std::runtime_error("Vulkan swapchain became out of date during acquire");
			}
			if (acquired.result != vk::Result::eSuccess &&
				acquired.result != vk::Result::eSuboptimalKHR) {
				throw vk::SystemError(acquired.result, "Vulkan acquireNextImageKHR failed");
			}
			...
			auto present_semaphore = scheduler.impl->device->createSemaphore(
				vk::SemaphoreCreateInfo{},
				nullptr,
				*scheduler.impl->dispatcher
			);
			vk::SharedSemaphore present_ready(
				present_semaphore,
				scheduler.impl->device,
				{ nullptr, *scheduler.impl->dispatcher }
			);
		...
		auto ReleaseRetirementFence = [&](PresentationWork const& work) {
			try {
				...
				queue.GetImplementation()->submit(
					vk::SubmitInfo{},
		...
		auto DiscardAcquiredPresentations = [&]() {
			...
			for (auto const& work : presentation_works) {
				...
				auto presentation = scheduler.impl->presentations.find(work.family);
				if (presentation == scheduler.impl->presentations.end()) {
					continue;
				}
				std::unique_lock<std::mutex> presentation_lock(presentation->second.mutex);
				presentation->second.cache.Erase(work.target);
			}
		};
		if (stop_token.stop_requested()) {
		...
		for (std::size_t resource = 0u; resource < plan.first_accesses.size(); ++resource) {
			...
			if (auto timeline = std::get_if<TimelineCompletion>(&state->completion)) {
				for (auto const& access : plan.first_accesses[resource]) {
		...
		if (scheduler.impl->timeline_semaphore_supported) {
			// Timeline values are reserved in plan order while execution_mutex is held.
			// Only this scheduler signals these semaphores, so next values need no atomics.
			auto& timelines = std::get<SchedulerContext::Implementation::TimelineMap>(
				scheduler.impl->completion_points
			);
			for (std::size_t index = 0u; index < prepared.size(); ++index) {
				...
				if (inserted) {
					...
					auto native = scheduler.impl->device->createSemaphore(
						info,
						nullptr,
						*scheduler.impl->dispatcher
					);
			...
			token.impl->binary_completions.reserve(
			...
			for (std::size_t index = 0u; index < prepared.size(); ++index) {
				...
				prepared[index].binary_completion = token.impl->binary_completions.size();
				token.impl->binary_completions.emplace_back(
					allocator->AllocateCompletionPoint(incoming)
				);
			}
			for (std::size_t index = 0u; index < prepared.size(); ++index) {
				auto destination = reservation.GetQueue(index).GetIdentifier();
				auto& completion = token.impl->binary_completions[
		...
		for (std::size_t resource = 0u; resource < plan.last_accesses.size(); ++resource) {
			...
			if (scheduler.impl->timeline_semaphore_supported) {
				...
				join.binary_completion = token.impl->binary_completions.size();
				token.impl->binary_completions.emplace_back(
					allocator->AllocateCompletionPoint(incoming)
				);
				auto& completion = token.impl->binary_completions.back();
		...
		tbb::parallel_for(
			...
			[&](std::size_t index) {
				...
				try {
					auto const& managed = token.impl->command_buffers[prepared[index].command_buffer];
					auto pool = scheduler.impl->command_pools.find(managed.family);
					...
					std::unique_lock<std::mutex> recording_lock(pool->second.mutex);
					...
					Recorder recorder{
						...
						*scheduler.impl->dispatcher,
						scheduler.impl->device,
						scheduler.impl->dynamic_rendering_supported,
						scheduler.impl->synchronization2_supported,
					...
					for (auto const& node : prepared[index].plan->nodes) {
						...
						for (std::size_t resource = 0u; resource < plan.first_accesses.size(); ++resource) {
							for (auto const& access : plan.first_accesses[resource]) {
								...
								if (auto texture = std::get_if<Resource::Texture>(&resources[resource].get().impl)) {
									stable.layout = texture->layout;
								}
						...
						for (std::size_t resource = 0u; resource < plan.last_accesses.size(); ++resource) {
							for (auto const& access : plan.last_accesses[resource]) {
								...
								if (stable.layout == vk::ImageLayout::eUndefined) {
									// UNDEFINED is illegal as a layout transition's new layout.
									// The texture's stable layout has not been established yet:
									// leave it in its last-access layout and adopt that layout as
									// the new stable one after submission (see the publish loop).
									continue;
								}
								EmitBarrier(
									commands,
					...
					commands.end(*scheduler.impl->dispatcher);
				}
				catch (...) {
		...
		for (auto const& batch : prepared) {
			if (batch.recording_error) {
				DiscardAcquiredPresentations();
				token.impl->exception = batch.recording_error;
				return token;
			}
		}
		if (cancelled.load(std::memory_order_acquire) || stop_token.stop_requested()) {
		...
		for (auto& batch : prepared) {
			for (auto& render_pass : batch.render_passes) {
				token.impl->render_passes.emplace_back(std::move(render_pass));
			}
			for (auto& framebuffer : batch.framebuffers) {
				token.impl->framebuffers.emplace_back(std::move(framebuffer));
			}
		}
		...
		tbb::parallel_for(
			...
			[&](std::size_t queue_index) {
				...
				for (auto index : batches) {
					...
					try {
						auto command_buffer = token.impl->command_buffers[prepared[index].command_buffer].impl;
						...
						if (prepared[index].binary_completion) {
							fence = *token.impl->binary_completions[
								*prepared[index].binary_completion
							].fence;
						}
						queue.GetImplementation()->submit(
						...
						if (prepared[index].timeline) {
							token.impl->command_buffers[prepared[index].command_buffer].completion =
		...
		for (std::size_t index = 0u; index < prepared.size(); ++index) {
			if (prepared[index].timeline && submission_states[index].load(std::memory_order_acquire) == 1u) {
				token.impl->timeline_completions.emplace_back(*prepared[index].timeline);
			}
			if (committed_work[index]) {
				token.impl->work_tokens.emplace_back(std::move(*committed_work[index]));
			}
		}
		for (auto const& batch : prepared) {
			if (batch.submission_error) {
				token.impl->exception = batch.submission_error;
				break;
			}
		}
		if (!presentation_works.empty()) {
			...
			for (auto const& [identifier, work_indices] : present_groups) {
				try {
					...
					auto aggregate = queue.GetImplementation()->presentKHR(
						present_info,
						*scheduler.impl->dispatcher
					);
					for (std::size_t position = 0u; position < work_indices.size(); ++position) {
						...
						if (result == vk::Result::eErrorOutOfDateKHR ||
							...
							if (presentation->second.cache.Contains(work.target)) {
								presentation->second.cache.Get(work.target).out_of_date = true;
							}
						}
						...
						token.impl->presentation_semaphores.emplace_back(work.present_ready);
						token.impl->presentation_fences.emplace_back(work.retirement_fence);
					}
				}
				catch (...) {
					if (!token.impl->exception) {
						token.impl->exception = std::current_exception();
					}
			...
			for (auto& join : resource_joins) {
				try {
					...
					if (join.binary_completion) {
						fence = *token.impl->binary_completions[*join.binary_completion].fence;
					}
					queue.GetImplementation()->submit(
						submit_info,
						fence,
					...
					if (join.timeline) {
						token.impl->timeline_completions.emplace_back(*join.timeline);
					}
					else {
						token.impl->binary_completions[*join.binary_completion].submitted = true;
					}
					join.submitted = true;
				}
				catch (...) {
		...
		for (std::size_t resource = 0u; resource < plan.last_accesses.size(); ++resource) {
				...
				state->owner_family = resource_anchors[resource];
				state->last_queue = queue_identifiers[completion_batch];
				if (device_drained_after_error) {
					state->completion = std::monostate{};
				}
				else if (joined != resource_joins.end()) {
					state->completion = joined->timeline
						? CompletionPoint(*joined->timeline)
						: CompletionPoint(BinaryCompletion(
							token.impl->binary_completions[*joined->binary_completion].fence
						));
				}
				else if (prepared[completion_batch].timeline) {
					state->completion = *prepared[completion_batch].timeline;
				}
				else {
					state->completion = BinaryCompletion(
						token.impl->binary_completions[
							*prepared[completion_batch].binary_completion
						].fence
					);
				}
				// A texture whose stable layout was UNDEFINED was left in its last-access
				// layout (the restore barrier above is skipped for UNDEFINED); adopt that
				// layout as the new stable one so the next graph can transition from it.
				std::visit(
					[&](auto& native) {
						using Native = std::remove_cvref_t<decltype(native)>;
						if constexpr (std::same_as<Native, Backend::Resource::Texture>) {
							if (native.layout == vk::ImageLayout::eUndefined) {
								native.layout = MapAccess(last.usage, last.mode).layout;
							}
						}
					},
					resources[resource].get().impl
