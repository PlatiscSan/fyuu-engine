...
namespace {
	...
	wgpu::LoadOp NativeLoadOp(LoadOperation operation) noexcept {
		...
		case LoadOperation::Discard: return wgpu::LoadOp::Clear;
		default: return wgpu::LoadOp::Load;
		}
	}
	...
	wgpu::StoreOp NativeStoreOp(StoreOperation operation) noexcept {
		...
		default: return wgpu::StoreOp::Store;
		}
	}
...
namespace fyuu_rhi::webgpu {
	...
	struct Replayer {
		...
		std::span<
			std::reference_wrapper<webgpu::PipelineResourceGroup const> const
		> groups;
...
namespace fyuu_rhi {
	...
	template <>
	struct CreateScheduler<webgpu::LogicalDevice> {
...
namespace fyuu_rhi::execution {
	...
	template <>
	struct ExecuteCommands<webgpu::CommandSchedulerContext> {
		...
		CompletionToken operator()(
			...
			std::vector<std::reference_wrapper<webgpu::PipelineResourceGroup const>> resource_groups;
			...
			std::ranges::transform(
				...
				[](View const& view) -> webgpu::View const& {
					...
					if (!native) {
						throw std::invalid_argument(
							"A WebGPU execution view uses another backend"
						);
					}
					return *native;
				}
			...
			std::ranges::transform(
				...
				[](PipelineResourceGroup const& group)
					-> webgpu::PipelineResourceGroup const& {
					if (!group.m_impl) {
						throw std::invalid_argument(
							"A WebGPU execution resource group is empty"
						);
					}
					auto native = std::get_if<webgpu::PipelineResourceGroup>(
						&group.m_impl->native
					);
					if (!native) {
						throw std::invalid_argument(
		...
		try {
			...
			for (auto const& batch : plan.batches) {
				for (auto const& node : batch.nodes) {
					for (auto const& command : node.commands) {
						...
						auto const& source = std::get<wgpu::Texture>(resources[present->source].get().impl);
						wgpu::SurfaceTexture current;
						wgpu::Surface surface;
						{
							std::lock_guard lock(context->surfaces_mutex);
							webgpu::CommandSchedulerContext::SurfaceState* surface_state = nullptr;
							...
							if (!surface_state) {
								...
								surface_state->surface = CreateSurface(
									context->instance,
			...
			for (auto const& batch : plan.batches) {
				...
				replayer.EndPass();
				presentation_cursor = replayer.presentation_cursor;
				command_buffers.emplace_back(encoder.Finish());
			}
			if (presentation_cursor != presentations.size()) {
				throw std::logic_error(
					"WebGPU execution did not consume every prepared presentation"
				);
			}
