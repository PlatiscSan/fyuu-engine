...
namespace fyuu_rhi {
	template <> struct MapResource<d3d12::Resource> {
		d3d12::Resource* resource;

		static void Unmap(void* context, ResourceDataRange range, bool writable) noexcept {
			auto native = static_cast<ID3D12Resource*>(context);
			D3D12_RANGE const written_range = writable
				? D3D12_RANGE{ range.offset, range.offset + range.size }
				: D3D12_RANGE{ 0u, 0u };
			native->Unmap(0u, &written_range);
		}

		ResourceMapScope operator()(ResourceDataRange range, bool writable) const {
			auto native = resource->allocation->GetResource();
			D3D12_RANGE const read_range = writable
				? D3D12_RANGE{ 0u, 0u }
				: D3D12_RANGE{ range.offset, range.offset + range.size };
			void* data = nullptr;
			d3d12::ThrowIfFailed(native->Map(0u, &read_range, &data));
			return ResourceMapScope(
			    native,
			    static_cast<std::byte*>(data) + range.offset,
			    range.offset,
			    range.size,
			    writable,
			    Unmap
	...
	template <> struct CreateBufferView<d3d12::Resource> {
		d3d12::Resource* resource;
		...
		View operator()(std::size_t offset, std::size_t range, ResourceFlags const& flags) const {
			...
			bool is_uav = flags.Test(Bits::StorageBuffer) || flags.Test(Bits::StorageTexelBuffer) ||
			...
			if (is_srv) {
				...
				if (format == DXGI_FORMAT_UNKNOWN) {
					...
					if ((offset % element_size) != 0u || (range % element_size) != 0u) {
						throw std::invalid_argument(
						    "A D3D12 typed buffer SRV offset and range must align to the element "
							"size"
						);
					}
				...
				result.descriptors[static_cast<std::size_t>(Type::ShaderResource)] =
				    std::move(handle);
			}
			if (is_uav) {
				auto handle = resource->resource_descriptors.Allocate();
				...
				if (format == DXGI_FORMAT_UNKNOWN) {
					...
					if ((offset % element_size) != 0u || (range % element_size) != 0u) {
						throw std::invalid_argument(
						    "A D3D12 typed buffer UAV offset and range must align to the element "
							"size"
						);
					}
				...
				result.descriptors[static_cast<std::size_t>(Type::UnorderedAccess)] =
				    std::move(handle);
			}
	...
	template <> struct CreateTextureView<d3d12::Resource> {
		d3d12::Resource* resource;
		...
		View operator()(
			...
			if (!is_srv && !is_uav && !is_rtv && !is_dsv) {
				throw std::invalid_argument("A D3D12 texture view requires a view type");
			}
			...
			if (is_srv) {
				...
				switch (srv_desc.ViewDimension) {
					...
					case D3D12_SRV_DIMENSION_TEXTURECUBEARRAY:
						if (arr_layer_cnt == 0u || base_arr_layer % 6u != 0u ||
						    arr_layer_cnt % 6u != 0u) {
							throw std::invalid_argument(
							    "A D3D12 cube array SRV requires base_array_layer and "
								"array_layer_count to be multiples of 6"
							);
						}
						...
						srv_desc.TextureCubeArray.First2DArrayFace =
						    static_cast<UINT>(base_arr_layer);
			...
			if (is_rtv) {
				...
				result.descriptors[static_cast<std::size_t>(Type::RenderTarget)] =
				    std::move(handle);
			}
			...
			if (is_dsv) {
				...
				result.descriptors[static_cast<std::size_t>(Type::DepthStencil)] =
				    std::move(handle);
			}
